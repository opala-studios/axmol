...
#include "OPInputPitchEstimator.hpp"
#include "../Debug/OPDebug.hpp"
#include "opalib/Audio/Juce/OPJuceEngine.hpp"

using namespace juce;

OPInputPitchEstimator::OPInputPitchEstimator(OPAudioEngine *manager):OPJuceInput(manager)
{
//    _desiredWindowSize = OPApplication::shared()->getAudioEngine()->getAudioBufferSize()*3;
    
    if(_desiredWindowSize <= 0)
    {
        _desiredWindowSize = 1536;
    }
    
    _inputDataInt = (int16_t*)malloc(sizeof(int16_t)*_desiredWindowSize);
    
    Yin_init(&_yin, _desiredWindowSize, 0.15);
    
    setRenderInputCallback([this](const float** incomingData, int numberOfSamples)
    {
        for(int i = 0; i < numberOfSamples; i++)
        {
            _inputDataInt[_numberOfFrames + i] = (int16_t)(incomingData[0][i]*32768.0);
        }
        
        _numberOfFrames += numberOfSamples;
        
        if(_numberOfFrames >= _desiredWindowSize)
        {
            float freq = Yin_getPitch(&_yin,_inputDataInt);
            float prob = Yin_getProbability(&_yin);
            if(freq > 0)
            {
                if(_pitchStimationCallback)
                {
                    _pitchStimationCallback(freq, prob);
                }
            }
            _numberOfFrames = 0;
        }
...
void OPInputPitchEstimator::setPitchStimationCallback(SL_PitchStimationCallback pitchStimationCallback)
{
    _pitchStimationCallback = pitchStimationCallback;
}
void OPInputPitchEstimator::Yin_difference(Yin *yin, int16_t* buffer)
{
    int16_t i;
    int16_t tau;
    float delta;
 ...
 */
void OPInputPitchEstimator::Yin_cumulativeMeanNormalizedDifference(Yin *yin)
{
    int16_t tau;
    float runningSum = 0;
    yin->yinBuffer[0] = 1;
 ...
 */
int16_t OPInputPitchEstimator::Yin_absoluteThreshold(Yin *yin)
{
    int16_t tau;
 ...
 * shift value.
 */
float OPInputPitchEstimator::Yin_parabolicInterpolation(Yin *yin, int16_t tauEstimate)
{
    float betterTau;
    int16_t x0;
    int16_t x2;
 ...
 */
void OPInputPitchEstimator::Yin_init(Yin *yin, int16_t bufferSize, float threshold)
{
 ...
 */
float OPInputPitchEstimator::Yin_getPitch(Yin *yin, int16_t* buffer)
{
    ...
    /* Step 5: Interpolate the shift value (tau) to improve the pitch estimate. */
    if(tauEstimate != -1){
        pitchInHertz = _sampleRate / Yin_parabolicInterpolation(yin, tauEstimate);
    }
 ...
 */
float OPInputPitchEstimator::Yin_getProbability(Yin *yin){
    return yin->probability;
}

OPInputPitchEstimator::~OPInputPitchEstimator() {
    free(_inputDataInt);
}
